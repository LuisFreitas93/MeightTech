...
# 3. Apply IQR
# The IQR is the range between the first quartile (25th percentile) and the third quartile (75th percentile) in a dataset. 
# Data points that fall outside of 1.5 times the IQR below the first quartile or above the third quartile are often considered outliers.
...
# 2. Create a cursor object to interact with Snowflake
cur = conn.cursor()

# 3. Ensure DataFrame columns match the Snowflake table
df = df[['service_type', 'pickup_coordinate', 'delivery_coordinate', 'price', 'trailer_type']]

# 4. Load the DataFrame into Snowflake (append)
